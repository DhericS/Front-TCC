...
export default function CadastroPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.telefone.match(/^\d{10,11}$/)) {
      setErro('Telefone inválido. Use somente números com DDD.');
      return;
    }
    ...
    if (form.tipoUsuario === 'personal' && !/^[0-9]{5,6}-[A-Z]{1,3}\/[A-Z]{2}$/.test(form.cref)) {
      setErro('CREF inválido. Use o formato 123456-G/SP');
      return;
    }
    ...
    >
      <div className="bg-black bg-opacity-70 text-white w-full max-w-6xl rounded-2xl shadow-xl p-6 flex flex-col md:flex-row">
        ...
        <div className="md:w-1/2 bg-white text-black rounded-xl p-6">
          ...
          {sucesso && <p className="text-green-600 text-sm mb-4">Cadastro realizado com sucesso!</p>}
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <button type="submit" className="w-full bg-black text-white py-2 px-4 rounded-md font-semibold border border-black hover:bg-white hover:text-black transition duration-300">
                Cadastrar
              </button>
            </div>
            <div className="col-span-2">
              <button type="submit" className="w-full bg-gray-500 text-white py-2 px-4 rounded-md font-semibold border border-black hover:bg-white hover:text-black transition duration-300" onClick={() => navigate('/login')}>
                Entrar
              </button>
            </div>
          </form>
